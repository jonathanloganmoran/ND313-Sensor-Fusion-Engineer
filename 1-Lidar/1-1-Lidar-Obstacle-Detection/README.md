...
### Clustering
* ⬜️ Extend the custom 2D `cluster::euclideanCluster()` algorithm to 3D for use on the _"Project 1.1" scene_ (see commit [``]() for reference);
* ⬜️ Extend the custom 2D `kdtree::KdTree` implementation to 3D for use on the _"Project 1.1" scene_ (see commit [``]() for reference);
...
### "Real-time" Detection
* ⬜️ Create new `environment::projectPipeline()` function to perform "complete" LiDAR scan processing pipeline (segmentation, clustering, downsampling, detection) across _multiple_ `.pcd` files from the _"Project 1.1" scene_ in a "streaming" manner (see commit [``]() for reference);
